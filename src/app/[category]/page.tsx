...
export default async function CategoryPage({ params }: { params: { category: string } }) {
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 capitalize">Today's Deals on {category.replace(/-/g, ' ')}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-8">
        {products.map((product) => {
          ...
          return (
            ...
            >
              <img
                src={product.productImageUrl ?? 'https://via.assets.so/shoe.png?id=1&q=95&w=180&h=180&fit=fill'}
                alt={product.title}
              ...
              <div className="p-4">
                <div className="mt-2 flex justify-between items-center">
                  <span className="text-red-600 font-medium text-xl">${product.currentPrice}</span>
